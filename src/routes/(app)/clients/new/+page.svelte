<script lang="ts">
  ...
  import {
    ...
    MapPin, Home, Layers, Maximize2,
    StickyNote, AlertCircle, ChevronRight,
    Hash, Check,
  } from 'lucide-svelte'
  ...
  // ─── Похідні ────────────────────────────────────────────
  const selectedTypeName = $derived(propTypes.find((t) => t.value === propType)?.label ?? '')
  ...
  <div class="mx-auto max-w-2xl px-4 sm:px-6">
    <div class="flex h-14 items-center justify-between gap-4">
      <div class="flex items-center gap-1.5 text-sm min-w-0">
        ...
        <span class="text-sm font-medium text-foreground truncate">Новий клієнт</span>
      </div>
      <Button size="sm" class="h-8 gap-1.5 cursor-pointer" onclick={createCustomer} disabled={loading}>
        {#if loading}
          <span class="h-3.5 w-3.5 border-2 border-current border-t-transparent rounded-full animate-spin"></span>
          Збереження...
        {:else}
  ...
  <div class="space-y-5">
      ...
      <div class="p-4 space-y-4">
        ...
        <div class="space-y-1.5">
          ...
          <div class="relative">
            ...
            <Input
              id="street"
              class="pl-9 h-9 text-sm {errors.street ? 'border-destructive focus-visible:ring-destructive' : ''}"
              bind:value={street}
        ...
        <div class="grid grid-cols-2 gap-3">
          <div class="space-y-1.5">
            ...
            <div class="relative">
              <Home class="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground pointer-events-none" />
              <Input id="apt" class="pl-9 h-9 text-sm" bind:value={apt} placeholder="45" />
            </div>
          </div>
          <div class="space-y-1.5">
            <Label for="floor" class="text-xs font-medium">Поверх</Label>
            <div class="relative">
              <Layers class="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground pointer-events-none" />
              <Input id="floor" type="number" min="0" class="pl-9 h-9 text-sm" bind:value={floor} placeholder="7" />
            </div>
          </div>
        </div>
        ...
        <div class="space-y-1.5">
          ...
          <div class="relative">
            <Maximize2 class="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground pointer-events-none" />
            <Input
              id="area"
              type="number"
              min="1"
          ...
          {:else}
            <p class="text-xs text-muted-foreground">Потрібно для розрахунку вартості</p>
          {/if}
        </div>
    ...
    <div class="rounded-xl border bg-muted/30 overflow-hidden shadow-sm">
      <div class="flex items-center gap-2 px-4 py-3 border-b bg-muted/40">
        ...
        <span class="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Підсумок</span>
      </div>
      <div class="p-4">
        <dl class="space-y-2.5">
          {#each [
            ...
            { label: 'Тип',     value: selectedTypeName },
            { label: 'Адреса',  value: fullAddress || '—' },
            { label: 'Площа',   value: area ? `${area} м²` : '—' },
          ] as row}
            <div class="flex items-baseline justify-between gap-4">
              <dt class="text-xs text-muted-foreground shrink-0">{row.label}</dt>
              <dd class="text-xs font-medium text-right truncate max-w-[220px]">{row.value}</dd>
            </div>
          {/each}
        </dl>
      </div>
